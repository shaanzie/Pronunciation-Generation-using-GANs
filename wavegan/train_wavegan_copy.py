...
def train(fps, args):
  ...
  # Make 2nd real discriminator
  with tf.name_scope('D2_x'), tf.variable_scope('D'):
    D2_x = WaveGANDiscriminator(x, **args.wavegan_d_kwargs)
  D2_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='D')
  ...
  # Make 2nd fake discriminator
  with tf.name_scope('D2_G_z'), tf.variable_scope('D', reuse=True):
    D2_G_z = WaveGANDiscriminator(G_z, **args.wavegan_d_kwargs)
  ...
  if args.wavegan_loss == 'dcgan':
    ...
    G_loss = (tf.reduce_mean((D_G_z - 1.) ** 2)) + (tf.reduce_mean((D2_G_z - 1.) ** 2))
    D_loss = tf.reduce_mean((D_x - 1.) ** 2)
    D_loss += tf.reduce_mean(D_G_z ** 2)
    D_loss = tf.reduce_mean((D2_x - 1.) ** 2)
    D_loss += tf.reduce_mean(D2_G_z ** 2)
    D_loss /= 2.
  elif args.wavegan_loss == 'wgan':
    G_loss = -tf.reduce_mean(D_G_z)
    D_loss = (tf.reduce_mean(D_G_z) - tf.reduce_mean(D_x)) + (tf.reduce_mean(D2_G_z) - tf.reduce_mean(D2_x))
    ...
    G_loss = (-tf.reduce_mean(D_G_z)) + -tf.reduce_mean(D2_G_z)
    D_loss = (tf.reduce_mean(D_G_z) - tf.reduce_mean(D_x)) + tf.reduce_mean(D2_G_z) - tf.reduce_mean(D2_x)
